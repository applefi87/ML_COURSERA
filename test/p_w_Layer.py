...
p_values = tf.random.uniform(shape=x_train.shape, minval=1.0, maxval=5.0)
...
class PowerLayer(Layer):
    ...
    def build(self, input_shape):
        self.w = self.add_weight(shape=(input_shape[-1], self.units),trainable=True,regularizer=regularizers.l2(0.01),name='w')
        self.p = self.add_weight(shape=(input_shape[-1], self.units),trainable=True,regularizer=regularizers.l2(0.01),name='p')
        super(PowerLayer, self).build(input_shape)
    def call(self, inputs):
        x_powered = power_fraction(inputs, self.p)
        x = self.w * x_powered
